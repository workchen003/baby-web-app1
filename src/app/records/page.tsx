...
export default function CareRecordsPage() {
    ...
    useEffect(() => {
        if (!userProfile?.familyIDs?.[0]) {
            if(!loading) router.push('/'); // 如果沒有家庭ID且不在載入中，則導向
            return;
        }
        ...
        return () => unsubscribe();
    }, [userProfile, loading, router, getTodayStartEnd]);

    // --- vvv 這裡是本次修改的重點 vvv ---
    // 步驟 1: 先用 useMemo 計算出基礎的統計數字
    ...
    // 步驟 2: 再用另外的 useMemo 基於這些基礎數字，產生給 UI 的資料
    ...
    const handleQuickAdd = async (type: CreatableRecordType, data: any) => {
        if (!userProfile?.familyIDs?.[0] || isSubmitting) return;
